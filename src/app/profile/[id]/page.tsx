...
import {
  Loader2, Settings, BookOpen, Heart, Star, FolderOpen, Users, UserPlus, UserMinus, Gamepad2, ChevronLeft
} from "lucide-react";
...
export default function UserProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [counts, setCounts] = useState({ library: 0, wishlist: 0, following: 0, followers: 0 });
  ...
  // 2. Load profile for the given ID
  useEffect(() => {
    if (!profileId) return;
    const load = async () => {
      setLoading(true);
      try {
        ...
        // Fetch all counts
        const reviewQ = query(collection(db, "reviews"), where("userId", "==", profileId));
        const colQ = query(collection(db, "collections"), where("userId", "==", profileId));
        const libQ = query(collection(db, "library"), where("userId", "==", profileId));
        const wishQ = query(collection(db, "wishlists"), where("userId", "==", profileId));
        const followingQ = query(collection(db, "follows"), where("followerId", "==", profileId));
        const followersQ = query(collection(db, "follows"), where("followingId", "==", profileId));
        
        const [reviewSnap, colSnap, libSnap, wishSnap, followingSnap, followersSnap] = await Promise.all([
          getCountFromServer(reviewQ),
          getCountFromServer(colQ),
          getCountFromServer(libQ),
          getCountFromServer(wishQ),
          getCountFromServer(followingQ),
          getCountFromServer(followersQ)
        ]);
        
        setCounts({
          library: libSnap.data().count,
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        if (tab === "library") {
          const q = query(collection(db, "library"), where("userId", "==", profileId));
          const snap = await getDocs(q);
          if (!cancelled) setLibrary(snap.docs.map(d => d.data() as { game: Game; status: string }));
        } else if (tab === "wishlist") {
          const q = query(collection(db, "wishlists"), where("userId", "==", profileId));
          const snap = await getDocs(q);
          if (!cancelled) setWishlist(snap.docs.map(d => (d.data() as { game: Game }).game));
        } else if (tab === "reviews") {
          const q = query(collection(db, "reviews"), where("userId", "==", profileId));
          const snap = await getDocs(q);
          if (!cancelled) setReviews(snap.docs.map(d => ({ id: d.id, ...d.data() })));
        } else if (tab === "following") {
          const q = query(collection(db, "follows"), where("followerId", "==", profileId));
          const snap = await getDocs(q);
          ...
          const q = query(collection(db, "follows"), where("followingId", "==", profileId));
          const snap = await getDocs(q);
          const users = await Promise.all(snap.docs.map(async d => {
            const data = d.data();
  ...
  if (!profileUser) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
      ...
      <Link href="/community" className="text-violet-500 hover:underline">Back to Community</Link>
    </div>
  );
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4 sm:gap-6 mb-6">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <h1 className="text-2xl sm:text-3xl font-outfit font-black text-foreground truncate">
                {profileUser.displayName}
              </h1>
              <UserBadge userId={profileUser.uid} role={profileUser.role} />
            </div>
            ...
            {tab === "overview" && (
              <div className="space-y-8">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {[
                    { label: "Library", value: counts.library, icon: <BookOpen className="w-5 h-5" /> },
                    { label: "Wishlist", value: counts.wishlist, icon: <Heart className="w-5 h-5" /> },
                    { label: "Following", value: counts.following, icon: <Users className="w-5 h-5" /> },
                    { label: "Followers", value: counts.followers, icon: <Users className="w-5 h-5" /> },
                  ].map((stat) => (
                  ...
                  {reviews.map((r) => (
                    <div key={r.id} className="bg-card border border-border rounded-xl p-5 hover:border-violet-500/30 transition-all flex flex-col h-full">
                      ...
                      <div className="mt-4 pt-3 border-t border-border flex justify-between items-center text-xs text-muted-foreground">
                        ...
                        {(r.likes ?? 0) > 0 && <span>{r.likes} likes</span>}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {following.map((u) => (
                    <Link key={u.uid} href={`/profile/${u.uid}`} className="flex items-center gap-3 p-4 bg-card border border-border rounded-2xl hover:border-violet-500/30 transition-all">
                      <div className="w-10 h-10 rounded-full bg-violet-600 flex items-center justify-center text-white font-bold shrink-0 relative overflow-hidden">
                        {u.photoURL ? (
                          <Image src={u.photoURL} alt="" fill className="object-cover rounded-full" />
                        ) : (
